...
import * as crypto from 'crypto'; // For SHA256 hashing
...
function initializeFirebase(context: vscode.ExtensionContext) {
    ...
    try {
        ...
        let serviceAccountPath = config.get<string>('firebaseServiceAccountPath');
        let databaseURL = config.get<string>('firebaseDatabaseURL');
...
async function createUserWithCode(): Promise<string | null> {
    if (!firebaseInitialized) {
        return null;
    }
...
// Renew user code
async function renewUserCode(oldCode: string): Promise<string | null> {
    if (!firebaseInitialized) {
        return null;
    }
    ...
    try {
        const db = admin.database();
        const usersRef = db.ref('users');
        const snapshot = await usersRef.orderByChild('specialCode').equalTo(oldCode).once('value');
...
async function saveMessageToSession(userId: string, sessionId: string, role: 'user' | 'assistant', content: string): Promise<boolean> {
    if (!firebaseInitialized) {
        return false;
    }
...
async function deleteSessionFromFirebase(userId: string, sessionId: string): Promise<boolean> {
    if (!firebaseInitialized) {
        return false;
    }
...
async function updateSessionTitle(userId: string, sessionId: string, newTitle: string): Promise<boolean> {
    if (!firebaseInitialized) {
        return false;
    }
...
let currntLoop = vscode.commands.registerCommand('acspl.CurrentLoop',async () => {
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('acspl.createFirebaseUser', async () => {
...
context.subscriptions.push(
    vscode.commands.registerCommand("acspl.monty", async () => {
...
- Always include STOP at the end of ACSPL+ code examples`;
            ...
            if (message.command === 'requestKey') {
                ...
                const subject = encodeURIComponent("Access Key Request - Monty");
                const body = encodeURIComponent("Hello,\n\nI would like to request an access key for Monty.\n\nThank you!");
                const mailtoLink = `mailto:barp@acsmotioncontrol.com?subject=${subject}&body=${body}`;
                vscode.env.openExternal(vscode.Uri.parse(mailtoLink));
                return;
            }
            ...
            if (message.command === 'authenticate') {
                ...
                if (isValid) {
                    ...
                    userId = await getUserIdFromCode(code);
                    ...
                    panel.webview.postMessage({
                        command: 'authResult',
                        success: true,
                        message: '✅ Authentication successful! Welcome to Monty.'
                    });
            ...
            if (message.command === 'sendMessage') {
                ...
                if (userId && currentSessionId) {
                    const existingMessages = await getSessionMessages(userId, currentSessionId);
                    isFirstMessage = existingMessages.length === 0;
                }

                // Save user message to Firebase
                if (userId && currentSessionId) {
                    await saveMessageToSession(userId, currentSessionId, 'user', userInput);

                    // Auto-generate title from first message
                    if (isFirstMessage) {
                        const newTitle = generateTitleFromMessage(userInput);
                        await updateSessionTitle(userId, currentSessionId, newTitle);
                ...
                if (uploadedPdfText) {
                    response = await queryAIWithPdf(`${systemIntro}\n\n${userInput}`, uploadedPdfText);
                } else {
                    response = await getAIResponse(`${systemIntro}\n\n${userInput}`);
                }

                // Save AI response to Firebase
                if (userId && currentSessionId) {
                    await saveMessageToSession(userId, currentSessionId, 'assistant', response);
                }
            ...
            if (message.command === 'uploadPDF') {
                ...
                try {
                    ...
                    if (message.query.trim()) {
                        const response = await queryAIWithPdf(`${systemIntro}\n\n${message.query}`, uploadedPdfText);
                        panel.webview.postMessage({ command: 'receiveMessage', text: response });
                    } else {
                        panel.webview.postMessage({ command: 'receiveMessage', text: '✅ PDF uploaded successfully! Now enter a query.' });
                    }
            ...
            if (message.command === 'loadSession') {
                if (userId && message.sessionId && currentSessionId) {
                    ...
                    const messages = await getSessionMessages(userId, message.sessionId);
            ...
            if (message.command === 'deleteSession') {
                if (userId && message.sessionId) {
                    const success = await deleteSessionFromFirebase(userId, message.sessionId);
...
context.subscriptions.push(
    vscode.commands.registerCommand("acspl.montyAI", async () => {
        console.log('=== MONTY AI COMMAND TRIGGERED ===');
...
function copyCode(buttonElement: HTMLElement) {
